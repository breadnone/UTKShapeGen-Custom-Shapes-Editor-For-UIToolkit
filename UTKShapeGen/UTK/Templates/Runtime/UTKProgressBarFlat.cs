...
namespace UTK
{
    public class UTKProgressBarFlat : VisualElement, INotifyValueChanged<float>
    {
        ...
        public UTKProgressBarFlat(float minValue, float maxValue, float defaultValue = 0)
        {
            if(minValue < 0)
                minValue = 0;
            ...
            containers.mainContainer.Size(100, 100, true, true).SetOverflow(Overflow.Hidden).FlexRow().Border(Utk.ScreenRatio(5), Utk.HexColor(Color.clear, "#0063b1")).BcgColor("#4c4a48");
            containers.mainContainer.AddChild(containers.bar);
            containers.bar.AddChild(containers.indicator);
            containers.bar.BcgColor("#0078d7").Size(defaultValue, 100, true, true).SetOverflow(Overflow.Hidden);
