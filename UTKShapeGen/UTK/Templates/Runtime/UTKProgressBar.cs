...
namespace UTK
{
    ...
    public class UTKProgressBar : VisualElement, INotifyValueChanged<float>
    {
        ...
        public UTKProgressBar(float minValue, float maxValue, float defaultValue = 0)
        {
            if(minValue < 0)
                minValue = 0;
            ...
            this.Size(Utk.ScreenRatio(50) * 6, Utk.ScreenRatio(50), false, false);
            this.minValue = minValue;
            this.maxValue = maxValue;
            this.AddChild(containers.mainContainer);
            containers.mainContainer.Size(100, 100, true, true).SetOverflow(Overflow.Hidden).FlexRow().Border(Utk.ScreenRatio(5), Utk.HexColor(Color.clear, "#0063b1")).RoundCorner(Utk.ScreenRatio(28), true).BcgColor("#4c4a48");
            containers.mainContainer.AddChild(containers.bar);
            containers.bar.AddChild(containers.indicator);
            containers.bar.RoundCorner(Utk.ScreenRatio(20), true).BcgColor("#0078d7").Size(defaultValue, 100, true, true).SetOverflow(Overflow.Hidden);
            containers.indicator.AlignSelf(Align.FlexEnd).TextAlignment(TextAnchor.MiddleRight).Height(100, true);
