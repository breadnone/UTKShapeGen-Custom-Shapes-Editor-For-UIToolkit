...
namespace UTK
{
    ...
    public class UTKTabStyleless : VisualElement
    {
        ...
        public UTKTabStyleless(List<string> tabs, bool isSideTab = false)
        {
            this.focusable = true;
            sideTab = isSideTab;
            this.FlexGrow(1);
        ...
        private void GenerateTabs()
        {
            ...
            for (int i = 0; i < _choices.Count; i++)
            {
                ...
                vis = new VisualElement().FlexGrow(1).Padding(2);
                ...
                tabs.Add((new Label().Text(_choices[i]).TextAlignment(TextAnchor.MiddleCenter).Size(width, height, true, true), vis, i, false, true));
        ...
        private void ShowTab(int index)
        {
            IterateTab(new Action<(Label tab, VisualElement content, int index, bool scrollMode, bool enable)>(x =>
            {
                ...
                if (tab.index == index)
                {
                    ...
                    tab.tab.Opacity(1f).Display(DisplayStyle.Flex).BorderRight(rightBorder, Utk.HexColor(Color.clear, "#744da9")).BorderBottom(topBorder, Utk.HexColor(Color.clear, "#744da9"));
        ...
        public UTKTabStyleless Rebuild()
        {
            Construct();
            return this;
        }
        ///<summary>Select next tab.</summary>
        public UTKTabStyleless Next()
        {
        ...
        public UTKTabStyleless Pervious()
        {
            if (selectedTab != tabs.Count - 1)
            {
                ShowTab(selectedTab - 1);
        ...
        public UTKTabStyleless JumpTo(int tabIndex)
        {
            tabIndex -= 1;
        ...
        public UTKTabStyleless InsertToTab(int tabIndex, VisualElement visualElement)
        {
